...
const GEMINI_COPY_BUTTON_SELECTOR = 'button[aria-label*="Copy"]';
...
const GEMINI_ATTACH_BUTTON_SELECTORS = [
...
const GEMINI_UPLOAD_MENU_TOGGLE_SELECTORS = [
  'button[aria-label="Open upload file menu"]',
  'button[aria-controls="upload-file-menu"]',
  'button.upload-card-button',
];
const GEMINI_UPLOAD_MENU_ITEM_SELECTORS = [
  'button[data-test-id="local-images-files-uploader-button"]',
  'button[aria-label*="Upload files"]',
  'images-files-uploader button[role="menuitem"]',
];
const AIMODE_UPLOAD_MENU_TOGGLE_SELECTORS = [
  'button[aria-label="More input options"]',
  'button.uMMzHc[aria-label*="More input options"]',
];
const AIMODE_UPLOAD_IMAGE_MENU_ITEM_SELECTORS = [
  'button[aria-label="Upload image"]',
  'button.nQpXxb[aria-label*="Upload image"]',
];
const AIMODE_ATTACH_BUTTON_SELECTORS = [
  'button[aria-label*="More input options"]',
  'button[aria-label*="Upload"]',
  'button[aria-label*="Attach"]',
  'button[aria-label*="Add files"]',
...
class BrowserAiInterface {
  ...
  async clickAttachButton(page, model) {
    const selectors = model === 'aimode' ? AIMODE_ATTACH_BUTTON_SELECTORS : GEMINI_ATTACH_BUTTON_SELECTORS;
    return this.clickBestButton(page, selectors, {
      allowHidden: false,
      keywords: ['upload', 'attach', 'add files', 'insert', 'image', 'photo', 'file'],
    });
  }

  async clickBestButton(page, selectors, options = {}) {
    const allowHidden = Boolean(options.allowHidden);
    const keywords = Array.isArray(options.keywords) ? options.keywords : [];
    const handle = await page.evaluateHandle((buttonSelectors, canUseHidden, keywordList) => {
      const isVisible = (el) => {
        if (!el) return false;
        const style = window.getComputedStyle(el);
        if (!style || style.display === 'none' || style.visibility === 'hidden') return false;
      ...
      const isClickable = (el) => {
        if (!el) return false;
        if (!canUseHidden && !isVisible(el)) return false;
      ...
      for (const selector of buttonSelectors || []) {
        document.querySelectorAll(selector).forEach((el) => {
          ...
          const rect = el.getBoundingClientRect();
          candidates.push({ el, bottom: Number(rect.bottom || 0) });
        });
      }
      if (!candidates.length && Array.isArray(keywordList) && keywordList.length) {
        ...
        pool.forEach((el) => {
          ...
          if (!label) return;
          if (!keywordList.some((kw) => label.includes(String(kw || '').toLowerCase()))) return;
          const rect = el.getBoundingClientRect();
          candidates.push({ el, bottom: Number(rect.bottom || 0) });
        });
      }
    ...
    }, selectors, allowHidden, keywords).catch(() => null);
  ...
  async uploadImagesViaFileChooser(page, model, files) {
    const accepted = Array.isArray(files) ? files.filter((p) => typeof p === 'string' && p.trim()) : [];
    if (!accepted.length) return 0;
    const chooserPromise = page.waitForFileChooser({ timeout: 3500 }).catch(() => null);
    let clicked = false;
    if (model === 'aimode') {
      const openedMenu = await this.clickBestButton(page, AIMODE_UPLOAD_MENU_TOGGLE_SELECTORS, {
        keywords: ['more input options'],
      });
      if (openedMenu) {
        await sleep(160);
        clicked = await this.clickBestButton(page, AIMODE_UPLOAD_IMAGE_MENU_ITEM_SELECTORS, {
          keywords: ['upload image'],
        });
      }
    } else {
      const openedMenu = await this.clickBestButton(page, GEMINI_UPLOAD_MENU_TOGGLE_SELECTORS, {
        keywords: ['open upload file menu', 'upload'],
      });
      if (openedMenu) {
        await sleep(180);
        clicked = await this.clickBestButton(page, GEMINI_UPLOAD_MENU_ITEM_SELECTORS, {
          keywords: ['upload files'],
        });
      }
    }
    if (!clicked) clicked = await this.clickAttachButton(page, model);
    if (!clicked) {
      await chooserPromise.catch(() => null);
      return 0;
    }
    const chooser = await chooserPromise;
    if (!chooser) return 0;
    await chooser.accept(accepted).catch(() => {});
    await sleep(350);
    return accepted.length;
  }

  async findUploadInput(page, preferAiMode = false) {
    const handle = await page.evaluateHandle((selector, preferAi) => {
      const all = Array.from(document.querySelectorAll(selector));
      if (!all.length) return null;
      const withContext = all.map((el) => ({
        el,
        inInputPlate: Boolean(el.closest('div[data-xid="aim-mars-input-plate"]')),
      }));
      let pool = withContext;
      if (preferAi) {
        const scoped = withContext.filter((v) => v.inInputPlate);
        if (scoped.length) pool = scoped;
      }
      return pool[pool.length - 1].el;
    }, FILE_INPUT_SELECTOR, preferAiMode).catch(() => null);
  ...
  async uploadImages(page, model, imagePaths) {
    const files = Array.isArray(imagePaths) ? imagePaths.filter((p) => typeof p === 'string' && p.trim()) : [];
    if (!files.length) return 0;

    const preferAiMode = model === 'aimode';
    let input = await this.findUploadInput(page, preferAiMode);
    if (!input) {
      const chooserCount = await this.uploadImagesViaFileChooser(page, model, files);
      if (chooserCount > 0) {
        this.log(`uploadImages: attached via file chooser count=${chooserCount} model=${model}`);
        return chooserCount;
      }
      await this.clickAttachButton(page, model);
      await sleep(350);
      input = await this.findUploadInput(page, preferAiMode);
    }
    if (!input) {
      throw new Error('Could not find file upload input for image attachment.');
    }

    const supportsMultiple = await input.evaluate((el) => Boolean(el.multiple)).catch(() => false);
    const accepted = supportsMultiple ? files : files.slice(0, 1);
    await input.uploadFile(...accepted);
    await sleep(350);
    await input.dispose().catch(() => {});
    return accepted.length;
  }

  async clickGeminiSendButton(page, timeoutMs = 5000) {
    const timeoutAt = Date.now() + Math.max(250, Number(timeoutMs) || 0);
    while (Date.now() < timeoutAt) {
      const clicked = await this.clickBestButton(page, GEMINI_SEND_SELECTORS, { keywords: ['send'] });
  ...
  async ensureGeminiModel(page, model) {
    if (model !== 'geminifast' && model !== 'geminithinking' && model !== 'geminipro') return false;
    ...
    const targetKeywords = model === 'geminifast'
      ? ['flash', 'fast']
      : ['thinking', 'advanced', 'pro', 'ultra'];
    const targetDataTestIds = model === 'geminifast'
      ? ['button[data-test-id="bard-mode-option-fast"]']
      : ['button[data-test-id="bard-mode-option-thinking"]', 'button[data-test-id="bard-mode-option-pro"]'];
    ...
    for (const selector of targetDataTestIds) {
      targetOption = await page.$(selector);
  ...
  async ensureGeminiThinkingSelected(page) {
    return this.ensureGeminiModel(page, 'geminithinking');
  }
  ...
  async sendPrompt(page, options) {
    ...
    let uploadedCount = 0;
    try {
      uploadedCount = await this.uploadImages(page, model, options && options.imagePaths ? options.imagePaths : []);
      if (uploadedCount > 0) {
        this.log(`sendPrompt: uploadedImages=${uploadedCount} model=${model}`);
      }
    } catch (err) {
      this.log(`sendPrompt: image upload skipped model=${model} reason=${err && err.message ? err.message : 'unknown'}`);
    }
    ...
    if (model === 'aimode') {
      ...
      const clicked = await this.clickGeminiSendButton(page, uploadedCount > 0 ? 8000 : 2500);
      this.log(`sendPrompt gemini: sendButtonClicked=${clicked} uploadedCount=${uploadedCount}`);
      if (!clicked) {
        await input.focus().catch(() => {});
        await page.keyboard.press('Enter');
        this.log('sendPrompt gemini: fallback Enter keypress sent');
        await sleep(150);
        const remainingLen1 = await page.evaluate((el) => {
          const v = 'value' in el ? String(el.value || '') : String(el.textContent || '');
          return v.trim().length;
        }, input).catch(() => 0);
        this.log(`sendPrompt gemini: inputRemainingLenAfterEnter=${remainingLen1}`);
        if (remainingLen1 > 0) {
          await input.focus().catch(() => {});
          await page.keyboard.down('Control').catch(() => {});
          await page.keyboard.press('Enter').catch(() => {});
          await page.keyboard.up('Control').catch(() => {});
          this.log('sendPrompt gemini: fallback Ctrl+Enter keypress sent');
          await sleep(150);
          const remainingLen2 = await page.evaluate((el) => {
            const v = 'value' in el ? String(el.value || '') : String(el.textContent || '');
            return v.trim().length;
          }, input).catch(() => 0);
          this.log(`sendPrompt gemini: inputRemainingLenAfterCtrlEnter=${remainingLen2}`);
          if (remainingLen2 > 0) {
            const retryClicked = await this.clickGeminiSendButton(page, 2000);
            this.log(`sendPrompt gemini: retrySendButtonClicked=${retryClicked}`);
          }
        }
      }
    }
  ...
  async waitForStableResponse(page, options) {
    ...
    while (Date.now() < timeoutAt) {
      ...
      if (responseStarted) {
        ...
        if (stableTicks >= 2) {
          const copied = await this.copyLatestGeminiResponseWithRetry(page, promptNorm, {
            baselineNorm: initialTextNorm,
            expectedNorm: candidateNorm,
          }, {
            attempts: 5,
            delayMs: 220,
          });
          this.log(`gemini copy attempt returned len=${copied.length}`);
          if (copied) return copied;
          this.log(`gemini fallback to candidate text len=${candidate.length}`);
          return candidate;
        }
      }
  ...
  async copyLatestGeminiResponseWithRetry(page, promptNorm = '', hints = null, options = null) {
    ...
    for (let i = 0; i < attempts; i += 1) {
      const copied = await this.copyLatestGeminiResponse(page, promptNorm, hints);
      if (copied) return copied;
  ...
  async copyLatestGeminiResponse(page, promptNorm = '', hints = null) {
    ...
    const copied = await page.evaluate(async ({ copySelector, containerSelector }) => {
      ...
      const isResponseCopyButton = (btn) => {
        if (!btn) return false;
        const label = String(btn.getAttribute('aria-label') || '').toLowerCase();
        if (!label.includes('copy')) return false;
        if (label.includes('code')) return false;
        return true;
      };

      let copyButtons = Array.from(latest.querySelectorAll(copySelector))
        .filter(isVisible)
        .filter(isResponseCopyButton)
        .sort((a, b) => a.getBoundingClientRect().bottom - b.getBoundingClientRect().bottom);
      if (!copyButtons.length) {
        copyButtons = Array.from(document.querySelectorAll(copySelector))
          .filter(isVisible)
          .filter(isResponseCopyButton)
          .sort((a, b) => a.getBoundingClientRect().bottom - b.getBoundingClientRect().bottom);
      }
      ...
      copySelector: GEMINI_COPY_BUTTON_SELECTOR,
      containerSelector: RESPONSE_CONTAINER_SELECTOR,
    }).catch(() => '');
    ...
    if (expectedNorm && tokenOverlapScore(norm, expectedNorm) < 0.2) return '';
    return normalized;
  }
