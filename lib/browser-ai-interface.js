...
const GEMINI_COPY_BUTTON_SELECTOR = 'button[aria-label="Copy"], button[aria-label="Copy response"], button[aria-label="Copy text"]';
...
class BrowserAiInterface {
  ...
  async waitForStableResponse(page, options) {
    ...
    while (Date.now() < timeoutAt) {
      ...
      if (responseStarted) {
        ...
        if (stableTicks >= 2) {
          const copied = await this.copyLatestGeminiResponse(page, promptNorm, {
  ...
  async copyLatestGeminiResponse(page, promptNorm = '', hints = null) {
    ...
    const copied = await page.evaluate(async ({ copySelector, containerSelector }) => {
      ...
      const copyButtons = Array.from(latest.querySelectorAll(copySelector))
        .filter(isVisible)
        .sort((a, b) => a.getBoundingClientRect().bottom - b.getBoundingClientRect().bottom);
